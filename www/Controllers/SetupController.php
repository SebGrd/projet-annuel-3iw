...
class SetupController
{
	...
	public function default()
	{
		...
		if (!empty($_POST) && $_SESSION['step'] == 3) {
			...
			$settings->setAppName($siteTitle);
			$settings->setVisibility($indexVisibility);
			$settings->save();
	...
	{
		$view = new View('admin.settings', 'blank');
		$settings = new Settings();
		$formS = $settings->formSettings();
		$view->assign('formS', $formS);

		if (!empty($_POST)) {
			// Fill out general website informations, and create the admin user
			$appName = htmlspecialchars(strip_tags($_POST['appName']));
			$visibility = isset($_POST['visibility']) ? 'true' : 'false';
			
			$envs = ['APPNAME' => $siteTitle, 'INDEX_VISIBILITY' => $indexVisibility];
			...
			Message::add('EDIT_SETTINGS_SUCCESS');
			$this->saveDotenv($envs);
			...
			header('Refresh:1; url=/admin', true, 303);
		}
	}
