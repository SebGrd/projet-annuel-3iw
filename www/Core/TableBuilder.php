...
class TableBuilder
{
    ...
    public static function render($class, array $excludeList = [])
    {
        ...
        $html .= self::generateRows($data, $columns, $instance->getModelName());
        $html .= '</table>';
        echo $html;
    }
    ...
    private static function generateRows($data, $titles, $modelName): string
    {
        ...
        foreach ($data as $row) {
            ...
            $html .= '<td style="width:120px">
                <a href="/'. $modelName .'?id=' . $row->id . '" class="btn btn-dark btn-icon-small">
                ...
                <a href="/admin/'. $modelName .'?id=' . $row->id . '" class="btn btn-primary btn-icon-small">
                ...
                <a href="/admin/'. $modelName .'?id=' . $row->id . '&action=delete" class="btn btn-danger btn-icon-small">
