    ...
    private static function generateRows($data, $titles, $modelName): string
    {
        ...
        if (!empty($data)) {
            foreach ($data as $row) {
                ...
                $html .= '<td class="flex justify-around" style="width:140px;">';
                if ($modelName == 'page') {
                    $html .= '<a href="/'. $modelName .'?id=' . $row->getId() . '" class="btn btn-dark btn-icon-small">
                ...
                $html .= '<a href="/admin/'. $modelName .'?id=' . $row->getId() . '" class="btn btn-primary btn-icon-small">
